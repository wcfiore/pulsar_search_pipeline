...
def main():
    ...
    parser.add_option("-c", "--numcpus", type="int", dest="numcpus", default=1,
                      help="Number of cpus/node working on this particular run.")
    ...
    for ii in range(len(dms)):
        ...
        if ii%options.N == options.run:
            ...
            ### zmax = 200 ###
            myexecute('accelsearch -ncpus %i -sigma %f -zmax 200 -numharm %i -flo %f -fhi %f ' % \
                    (options.numcpus, options.sigma, options.numharm, options.flo, options.fhi)+filenamebase+'.fft')
